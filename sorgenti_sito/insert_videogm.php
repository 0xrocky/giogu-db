<?php
	...
	if ( isSet($_POST['game']) && isSet($_POST['nplayer']) && isSet($_POST['age']) ) {
		...
		if ( !empty($_POST['game']) && !empty($_POST['nplayer']) && !empty($_POST['age']) && !empty($_POST['console']) ) {
			...
			if (!(is_numeric($_POST['nplayer'])) || !(is_numeric($_POST['age']))) {
				...
				if ($row >= 1) {
					...
					print ("<form class=\"padding-el\" action=\"insert_videogm.php?game=$game&player=$player&age=$age\" method=\"post\">");
						foreach ($_POST['console'] as $value) {
							print ("<input type=\"hidden\" name=\"console[]\" value=\"$value\"/>");
						}
				...
				/* Else insert it directly */
					...
					pg_free_result($resource);
					$resource = $db->query("INSERT INTO Game(name, sugg_age, n_player, type) VALUES('$game','$age','$player','0')");
					pg_free_result($resource);
					/* Recover gid to insert the game in the specific typology table */
					$resource = $db->query("SELECT gid FROM Game WHERE name='$game' AND sugg_age='$age' AND n_player='$player' AND type='0'");
					$result = pg_fetch_array($resource,NULL,PGSQL_ASSOC);
					$gid = $result['gid'];
					pg_free_result($resource);
					$resource = $db->query("INSERT INTO Videogame VALUES('$gid')");
					pg_free_result($resource);
					foreach ($_POST['console'] as $value) {
						$resource = $db->query("INSERT INTO Reference VALUES('$gid','$value')");
						pg_free_result($resource);
					}
					print("<p>Click <a href=\"profile_game.php?gid=$gid&type=0\" target=\"_parent\">here</a> to get profile of new game.</p>");
				}
			}
		}
		else {	/* incomplet data */
	...
	/* If parameters arrive via GET, then existed a simil game, so the user has sent his intention via POST */
			...
			if ($_POST['button']=='Yes') {
				...
				$resource = $db->query("SELECT gid FROM Game WHERE name='{$_GET['game']}' AND type='0'");
				$row = pg_num_rows($resource);
				if ($row == 1) {
					$result = pg_fetch_array($resource,NULL,PGSQL_ASSOC);
					print("<p>This game already exists! Click <a href=\"profile_game.php?gid={$result['gid']}&type=0\" target=\"_parent\">here</a> to get profile of it.</p>");			
					pg_free_result($resource);
				}
				else {
					/* Insert game */
					$resource = $db->query("INSERT INTO Game(name, sugg_age, n_player, type) VALUES('{$_GET['game']}','{$_GET['age']}','{$_GET['player']}','0')");
					pg_free_result($resource);
					/* Recover gid to insert the game in the specific typology table */
					$resource = $db->query("SELECT gid FROM Game WHERE name='{$_GET['game']}' AND sugg_age='{$_GET['age']}' AND n_player='{$_GET['player']}' AND type='0'");
					$result = pg_fetch_array($resource,NULL,PGSQL_ASSOC);
					$gid = $result['gid'];
					pg_free_result($resource);
					$resource = $db->query("INSERT INTO Videogame VALUES('$gid')");
