<?php
	...
	if (!empty($_POST['game'])) {
		...
		if ($row == 1) {
			...
			if ($row >= 1) {
				pg_free_result($resource);
				$resource = $db->query("DELETE FROM Association WHERE gid='$gid'");
				print("<p>All Tags dissociated from $game. Click <a href=\"../index.php?mod=moder\" target=\"_parent\">here</a> to continue to moderate.</p>");
			}
			else print("<p>Any Tag hasn't associated with this game. Please come <a href=\"../index.php?mod=moder\" target=\"_parent\"> back </a> and re-insert.</p>");
			pg_free_result($resource);
		}
		else {
